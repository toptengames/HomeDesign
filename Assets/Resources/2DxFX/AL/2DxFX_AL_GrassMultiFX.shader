  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: FORWARD
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat1_d.xy = float2(((u_xlat6_d.xz * float2(_Wind2, _Wind3)) + u_xlat1_d.xz));
          u_xlatb11.xy = float2(greaterThanEqual(abs(u_xlat1_d.xyxy), (-abs(u_xlat1_d.xyxy))).xy);
          ...
          if((abs(u_xlat1_d.x)>=(-abs(u_xlat1_d.x))))
          {
              u_xlatb6 = 1;
          }
          else
    ...
    Pass // ind: 3, name: PREPASS
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat1_d = (in_f.texcoord.xyxy + float4(0.200000003, 0.00999999978, 0.400000006, 0.0199999996));
          u_xlat3.xy = float2((u_xlat1_d.yw / float2(_Distortion, _Distortion)));
          u_xlat3.xy = float2(((u_xlat3.xy * float2(_Wind2, _Wind3)) + u_xlat1_d.xz));
          u_xlatb1.xy = float2(greaterThanEqual(abs(u_xlat3.xyxx), (-abs(u_xlat3.xyxx))).xy);
          u_xlat3.xy = float2(frac(abs(u_xlat3.xy)));
          u_xlat1_d.x = (u_xlatb1.x)?(u_xlat3.x):((-u_xlat3.x));
          u_xlat1_d.y = (u_xlatb1.y)?(u_xlat3.y):((-u_xlat3.y));
          u_xlat1_d.zw = (in_f.texcoord.yy + float2(0.00999999978, 0.0199999996));
          u_xlat10_3 = tex2D(_MainTex, u_xlat1_d.xz).w.x;
          u_xlat10_6 = tex2D(_MainTex, u_xlat1_d.yw).w.x;
          ...
          if((abs(u_xlat3.x)>=(-abs(u_xlat3.x))))
          {
              u_xlatb6 = 1;
          }
          else
    ...
    Pass // ind: 4, name: PREPASS
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat1_d.xy = float2(((u_xlat6.xz * float2(_Wind2, _Wind3)) + u_xlat1_d.xz));
          u_xlatb11.xy = float2(greaterThanEqual(abs(u_xlat1_d.xyxy), (-abs(u_xlat1_d.xyxy))).xy);
          u_xlat1_d.xy = float2(frac(abs(u_xlat1_d.xy)));
          float4 hlslcc_movcTemp = u_xlat1_d;
          hlslcc_movcTemp.x = (u_xlatb11.x)?(u_xlat1_d.x):((-u_xlat1_d.x));
    ...
    Pass // ind: 6, name: ShadowCaster
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat3_d.xy = float2((u_xlat1_d.yw / float2(_Distortion, _Distortion)));
          u_xlat3_d.xy = float2(((u_xlat3_d.xy * float2(_Wind2, _Wind3)) + u_xlat1_d.xz));
          u_xlatb1.xy = float2(greaterThanEqual(abs(u_xlat3_d.xyxx), (-abs(u_xlat3_d.xyxx))).xy);
          u_xlat3_d.xy = float2(frac(abs(u_xlat3_d.xy)));
          ...
          if((abs(u_xlat3_d.x)>=(-abs(u_xlat3_d.x))))
          {
              u_xlatb6 = 1;
          }
          else
