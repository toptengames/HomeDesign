  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat1_d.xyz = float3(((u_xlat10_0.xyz * in_f.color.xyz) + (-_ColorX.xyz)));
          u_xlatb1.xyz = float3(lessThan(abs(u_xlat1_d.xyzx), float4(float4(_Tolerance, _Tolerance, _Tolerance, _Tolerance))).xyz);
          ...
          u_xlat9.xy = float2(((float2(float2(_Val, _Val)) * float2(0.298999995, 0.587000012)) + (-u_xlat2.yz)));
          u_xlat2.xy = float2(((float2(float2(_Val, _Val)) * float2(0.587000012, 0.114)) + u_xlat2.xw));
          ...
          out_f.color.xyz = float3((u_xlatb1.x)?(u_xlat2.xyz):(u_xlat0_d.xyz));
          return out_f;
      }
